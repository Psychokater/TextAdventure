...
def Encounter(startLocation, location, playerStats, playerStatPoints, playerInventoryMoney, playerName, itemsDict):
   ...
   # Playerstats = 0 Level, 1 MAX HP, 2 HP, 3 ATK, 4 DEF, 5 EXP 
    locationIndex = 0
    enemyID = 0
    _locations = [startLocation, "the town", "the flatlands", "the forest", "the mountains", "the islands","mystic stones", "the castle","green land","the desert","the lake"]
    _locationIndexList = [1,          100,             1,              1,            1,             1,          200,          201,         1,           1,          1     ]
    ...
    for i in range(0,len(_locations)):
        if location == _locations[i]:
            ...
            if locationIndex == 200:
                itemsDict, playerInventoryMoney, playerStats = Inventory.DragonMerchantMenu(
                    startLocation, location, itemsDict, playerName, playerInventoryMoney, playerStats) 
            
                
    if playerInventoryMoney >= 60:
        luck = random.randint(13,113)
    else:
    ...
    if enemyID != 0:
        ...
        while True:           
            ...
            f"---------------------- VS ----------------------\n"\
            f"{strength} {cl.RED}{'{:<15}'.format(selectedDict[enemyID][0])}{cl.RESET}\t\tLVL {cl.BLUE}{selectedDict[enemyID][5]}{cl.RESET}\tHP {cl.BLUE}{round(selectedDict[enemyID][2],2)}/{round(enemyMaxHP,2)}{cl.RESET}\n {rank}\n\n"\
...
#Enemy: 0 Name, 1 LVL, 2 HP, 3 ATK, 4 DEF, 5 Dropvalue, 6 Pic 
    ...
    for y in range(1,3):                                                                                    
        ...
        for k in itemsDict:
            ...
            if itemsDict[k][10] > 0 and itemsDict[k][10] < 13:                      # from all choosen Items, append one Primary and Secondary Equipment to enemyItems
